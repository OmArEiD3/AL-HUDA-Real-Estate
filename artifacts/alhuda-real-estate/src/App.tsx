...
function PropertyCard({ property, language }: { property: Property; language: Language }) {
  ...
  return (
    <Link href={`/properties/${property.id}`} className="property-card group block overflow-hidden rounded-[1.1rem]" data-testid={`card-property-${property.id}`}>
      <div className="relative h-64"><ImageFrame property={property} className="h-full w-full" /><div className="image-overlay image-overlay-strong absolute inset-0" /><div className="absolute left-4 right-4 top-4 flex items-start justify-between" dir="ltr"><span className="rounded-full bg-[#f7f0e4]/90 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-[#143f42]">{property.operation === 'rent' ? t.rent : t.sale}</span>{property.featured && <span className="rounded-full bg-[#d85e45] px-2.5 py-1 text-[10px] font-bold text-white"><Star size={11} className="mr-1 inline" />{language === 'ar' ? 'مختار' : 'Featured'}</span>}</div><div className="absolute bottom-4 left-4 right-4 text-[#fff8ec]" dir={language === 'ar' ? 'rtl' : 'ltr'}><p className="mb-1 flex items-center gap-1 text-xs text-white/80"><MapPin size={12} />{property.neighborhood}{property.compound ? ` · ${property.compound}` : ''}</p><h3 className="line-clamp-2 text-lg font-bold leading-snug sm:text-xl">{title}</h3></div></div>
...
function App() {
  ...
  return <QueryClientProvider client={queryClient}><WouterRouter base={import.meta.env.BASE_URL.replace(/\/$/, '')}><Router language={language} onLanguageChange={setLanguage} /></WouterRouter></QueryClientProvider>;
}
